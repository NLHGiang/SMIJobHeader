...
public class RedisCacheService : RedisCache
{
        ...
        foreach (var item in _options.ConfigurationOptions.EndPoints)
        {
            ...
            foreach (var element in data)
            {
                ...
                if (element.StartsWith(_options.InstanceName))
                    key = element.Substring(_options.InstanceName.Length,
                        element.Length - _options.InstanceName.Length);
    ...
    public async Task<List<string>> GetHashFieldAsync(string key, CancellationToken token = default)
    {
        ...
        foreach (var i in response) result.Add(i.Name.ToString());
        return result;
    }

    public async Task HashSetAsync<T>(string key, Dictionary<string, T> data,
        DistributedCacheEntryOptions options = null, CancellationToken token = default)
    {
        ...
        await _cache.HashSetAsync(key,
            data.Select(x => new HashEntry(x.Key, JsonConvert.SerializeObject(x.Value))).ToArray());
    ...
    public async Task HashSetAsync(string key, Dictionary<string, string> data,
        DistributedCacheEntryOptions options = null, CancellationToken token = default)
    {
        if (key == null)
            throw new ArgumentNullException(nameof(key));
    ...
    public async Task HashSetAsync(string key, string hashField, string data,
        DistributedCacheEntryOptions options = null, CancellationToken token = default)
    {
        if (key == null)
            throw new ArgumentNullException(nameof(key));
    ...
    public async Task<T> QueryCacheKeyAsync<T>(string cache, Func<Task<T>> query,
        DistributedCacheEntryOptions options = null, CancellationToken token = default)
    {
    ...
    public async Task<T> QueryHashKeyAsync<T>(string cache, string key, Func<Task<T>> query,
        DistributedCacheEntryOptions options = null, CancellationToken token = default)
    {
    ...
    public async Task<List<T>> QueryHashKeysAsync<T>(string cache, List<string> keys, Func<Task<List<T>>> query,
        Func<List<T>, Dictionary<string, T>> parser, DistributedCacheEntryOptions options = null,
    ...
    public async Task<string> StringGetAsync(string key, CommandFlags commandFlags = CommandFlags.None,
        CancellationToken token = default)
    {
        if (key == null)
            throw new ArgumentNullException(nameof(key));
    ...
    public async Task StringSetAsync(string key, string data, TimeSpan? expiry,
        CommandFlags commandFlags = CommandFlags.None, CancellationToken token = default)
    {
        if (key == null)
            throw new ArgumentNullException(nameof(key));
