...
namespace SMIJobHeader.DBAccessor.Interface;
...
public interface IRepository<T>
{
    ...
    IEnumerable<T> Filter(FilterDefinition<T> combineFilters, SortDefinition<T> sort, int startingFrom = 0,
        int count = int.MaxValue);

    IEnumerable<T> Filter(List<FilterDefinition<T>> filters, SortDefinition<T> sort, int startingFrom = 0,
        int count = int.MaxValue);
