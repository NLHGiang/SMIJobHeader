...
namespace SMIJobXml.DBAccessor.Interface.Repositories
{
    public interface IMongoRepository<TEntity, T> where TEntity : class, IBaseEntity<T>
    {
        ...
        IEnumerable<TEntity> Filter(FilterDefinition<TEntity> combineFilters, SortDefinition<TEntity> sort, int startingFrom = 0, int count = int.MaxValue);
        ...
        IEnumerable<TEntity> Filter(List<FilterDefinition<TEntity>> filters, SortDefinition<TEntity> sort, int startingFrom = 0, int count = int.MaxValue);
