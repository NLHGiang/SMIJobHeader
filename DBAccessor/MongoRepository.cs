    ...
    where TEntity : class, IBaseEntity<T>
{
    ...
    public IEnumerable<TEntity> Filter(FilterDefinition<TEntity> combineFilters, SortDefinition<TEntity> sort,
        int startingFrom = 0, int count = int.MaxValue)
    {
        if (sort == null) return _moderCollection.Find(combineFilters).Skip(startingFrom).Limit(count).ToList();

        return _moderCollection.Find(combineFilters).Sort(sort).Skip(startingFrom).Limit(count).ToList();
    }

    public IEnumerable<TEntity> Filter(List<FilterDefinition<TEntity>> filters, SortDefinition<TEntity> sort,
        int startingFrom = 0, int count = int.MaxValue)
    {
        if (filters == null || filters.Count == 0) return Filter(sort, startingFrom, count);
    ...
    {
        await _moderCollection.InsertOneAsync(data);
    }

    public async Task InsertMany(IEnumerable<TEntity>? data)
    {
        await _moderCollection.InsertManyAsync(data);
    }
    ...
    {
        await _moderCollection.DeleteOneAsync(combineFilters);
        await _moderCollection.InsertOneAsync(data);
    }

    public bool Exist(FilterDefinition<TEntity> filter)
    {
        return _moderCollection.Find(filter).Any();
    }
}