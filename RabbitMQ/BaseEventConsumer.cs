...
public abstract class BaseEventConsumer<T> : BackgroundService, IDistributedEvent<T>
{
    ...
    public override Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        Channel.QueueDeclare(DeclareOption.Queue.Name, DeclareOption.Queue.Durable, DeclareOption.Queue.Exclusive,
            DeclareOption.Queue.AutoDelete, DeclareOption.Queue.Arguments);

        foreach (var item in DeclareOption.Binding)
        {
            Channel.ExchangeDeclare(item.Exchange.Name, item.Exchange.Type, item.Exchange.Durable,
                item.Exchange.AutoDelete, item.Exchange.Arguments);
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.Received += async (sender, args) =>
        {
            ...
            try
            {
                ...
                var parentContext =
                    Propagator.Extract(default, args.BasicProperties, ExtractTraceContextFromBasicProperties);
                Baggage.Current = parentContext.Baggage;

                // Start an activity with a name following the semantic convention of the OpenTelemetry messaging specification.
                // https://github.com/open-telemetry/semantic-conventions/blob/main/docs/messaging/messaging-spans.md#span-name
                using var activity = ActivitySource.StartActivity("ReceiveMessage", ActivityKind.Consumer,
    ...
    {
        Channel.BasicAck(Args.DeliveryTag, multiple);
    }
